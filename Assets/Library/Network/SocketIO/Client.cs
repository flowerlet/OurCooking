...
{
    /// <summary>
    /// Class to emulate SocketEntity.io javascript client capabilities for .net classes
    /// </summary>
    ...
    public class Client : IDisposable, IClient
    {
        ...
        private static readonly object padLock = new object(); // allow one connection attempt at a time
        ...
        // Events
        /// <summary>
        /// Opened event comes from the underlying websocket client connection being opened.  This is not the same as SocketEntity.io returning the 'connect' event
        /// </summary>
        ...
        /// <summary>
        /// Represents the initial handshake parameters received from the SocketEntity.io service (SID, HeartbeatTimeout etc)
        /// </summary>
        ...
        */
        ...
        public Client(string url)
        {
            ...
            this.dequeuOutBoundMsgTask = new Thread(new ThreadStart(DequeueOutboundMessages));
                    ...
                    try
                    {
                        ...
                        this.HandShake =
                            this.RequestHandshake(
                                uri); // perform an initial HTTP request as a new, non-handshaken connection

                        if (this.HandShake == null || string.IsNullOrEmpty(this.HandShake.SID) ||
                        ...
                        else
                        {
                            ...
                            this.wsClient = new WebSocket(
                                string.Format("{0}://{1}:{2}/SocketEntity.io/1/websocket/{3}", wsScheme, uri.Host, uri.Port, this.HandShake.SID),
                                string.Empty,
                                this.socketVersion);
                            ...
                            */
                            this.wsClient = new WebSocket(
                                $"{wsScheme}://{uri.Host}:{uri.Port}/SocketEntity.io/1/websocket/{this.HandShake.SID}");
        ...
        /// <para>Mimicks the Socket.IO client 'SocketEntity.on('name',function(data){});' pattern</para>
        /// <para>Reserved SocketEntity.io event names available: connect, disconnect, open, close, error, retry, reconnect  </para>
        /// </summary>
        ...
        /// <para>Mimicks Socket.IO client 'SocketEntity.emit('name',payload);' pattern</para>
        /// <para>Do not use the reserved SocketEntity.io event names: connect, disconnect, open, close, error, retry, reconnect</para>
        /// </summary>
        /// <param name="eventName"></param>
            ...
            switch (lceventName)
            {
                ...
                case "reconnect":
                    throw new System.ArgumentOutOfRangeException(eventName,
                        "Event name is reserved by SocketEntity.io, and cannot be used by clients or servers with this message type");
                default:
        ...
        {
            this.MessageQueueEmptyEvent.Reset();
            if (this.outboundQueue != null) this.outboundQueue.Enqueue(rawEncodedMessageText);
        }
        ...
        protected void closeWebSocketClient()
        {
            if (this.wsClient != null)
            {
                ...
                */
                ...
                if (this.wsClient.ReadyState == WebSocketState.Connecting ||
                    this.wsClient.ReadyState == WebSocketState.Open)
                {
                    try
                    {
                        this.wsClient.Close();
                    ...
                    {
                        Trace.WriteLine(
                            "exception raised trying to close websocket: can safely ignore, SocketEntity is being closed");
                    }
                }
        ...
        {
            this.socketHeartBeatTimer = new Timer(OnHeartBeatTimerCallback, new object(), HandShake.HeartbeatInterval,
        ...
        private void wsClient_MessageReceived(object sender, WebSocketSharp.MessageEventArgs e)
        {
            ...
            IMessage iMsg = Messages.Message.Factory(e.Data);
            ...
            switch (iMsg.MessageType)
            {
                case SocketIOMessageTypes.Disconnect:
                    ...
                    if (string.IsNullOrEmpty(iMsg.Endpoint)) // Disconnect the whole SocketEntity
                        this.Close();
                    break;
        ...
        /// <summary>
        /// While connection is open, dequeue and send messages to the SocketEntity server
        /// </summary>
        protected void DequeueOutboundMessages()
        {
        ...
        protected SocketIOHandshake RequestHandshake(Uri uri)
        {
            ...
            using (WebClient client = new WebClient())
            {
                try
                {
                    ...
                    value = client.DownloadString(string.Format("{0}://{1}:{2}/SocketEntity.io/1/{3}", uri.Scheme, uri.Host,
                        uri.Port,
                        uri.Query)); // #5 tkiley: The uri.Query is available in SocketEntity.io's handshakeData object during authorization
                ...
                {
                    errorText = $"Error getting handshake from Socket.IO host instance: {ex.Message}";
